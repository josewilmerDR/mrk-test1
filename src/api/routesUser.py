...
# 1 - REGISTRO DE USUARIO.
# VER DOCUMENTACION ADICIONAL SOBRE ESTA RUTA EN: https://www.notion.so/dicttaapp-1-REGISTRO-DE-USUARIO-7ed225b8b61a4461a68413a37253434cs = URLSafeTimedSerializer("any key works")
...
def register_user():
    ...
    # Verificamos que todos los campos requeridos estén presentes
    for field in ["email", "name", "lastname", "password"]:
    ...
    if not validate_password(password):
        raise APIException(
            {
                "message": "La contraseña debe tener al menos 8 caracteres, incluyendo una letra mayúscula, una letra minúscula, un número y al menos un simbolo especial (@#$%^&+=*/)"
            },
            status_code=400,
        )
    ...
    if existing_user:
        raise APIException(
            {"message": "El correo electrónico ya está registrado"}, status_code=400
        )
