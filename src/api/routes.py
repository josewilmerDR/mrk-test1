...
def handle_hello():
    ...
    response_body = {"message": password_encrypted}
...
def signup():
    ...
    try:
        if body is None:
            raise APIException(
                "Body está vacío o email no viene en el body, es inválido",
                status_code=400,
            )
...
def open_ai():
    body = request.get_json()
    prompt = (
        "Eres una página web de citas para cuidar mascotas, responde acorde a esto, según lo que te pregunte el usuario: "
        + body["prompt"]
    )
    ...
    response = {"message": completation.choices[0].text}
